...
async function run() {
  try {
    ...
    app.post("/club", async (req, res) => {
      ...
      const exist = await UniversityCollection.findOne({
        _id: new ObjectId(club.universityId),
      });
      if (exist) {
        const isCLub = await ClubCollection.findOne(
          { name: club.name },
          { projection: { _id: 1 } }
        );
        //  console.log(isCLub)
        if (isCLub) console.log("CLUB IS ALREADY EXIST");
        else {
    ...
    app.post("/users", async (req, res) => {
      ...
      //    console.log(isExist)
      const university_id = await UniversityCollection.findOne(
    ...
    app.patch("/users/:email", async (req, res) => {
      ...
      if (name) {
        const result = await userCollestion.updateOne(
          { email: email },
          { $set: { name: name } }
        );
        res.send(result);
    ...
    app.patch("/event/:id", async (req, res) => {
      ...
      try {
        ...
        if (result.modifiedCount > 0) {
          ...
          res
            .status(404)
            .json({ error: "Event not found or status unchanged" });
        }
      } catch (err) {
    ...
    app.put("/participants", async (req, res) => {
      ...
      const isExist = await participantCollection.findOne({
    ...
    app.delete("/wishlist/:id", async (req, res) => {
      ...
      try {
        ...
        if (result.deletedCount > 0) {
          ...
          res
            .status(404)
            .send({ success: false, message: "Wishlist item not found" });
        }
      } catch (error) {
    ...
    app.post("/create-ssl-payment", async (req, res) => {
      ...
      const initiate = {
        ...
        tran_id: trxid,
        success_url:
          "https://club-event-management-server.onrender.com/success-payment",
        ...
        ipn_url:
          "https://club-event-management-server.onrender.com/ipn-success-payment",
    ...
    app.get("/payments/:email/:trxid", async (req, res) => {
      ...
      try {
        const payment = await paymentCollection
          .find({ userEmail: email, trxId: trxid })
          .toArray();
        res.send(payment);
      } catch (error) {
        console.error(error);
    ...
    app.get("/success-payment", async (req, res) => {
      try {
        ...
        const isValid = await axios.get(
          `https://sandbox.sslcommerz.com/validator/api/validationserverAPI.php?val_id=${val_id}&store_id=mysha67a089e21f898&store_passwd=mysha67a089e21f898@ssl&v=1&format=json`
        );
    ...
    app.post("/success-payment", async (req, res) => {
      try {
        ...
        const isValid = await axios.get(
          `https://sandbox.sslcommerz.com/validator/api/validationserverAPI.php?val_id=${successPay.val_id}&store_id=mysha67a089e21f898&store_passwd=mysha67a089e21f898@ssl&v=1&format=json`
        );
        ...
        const uni = await userCollestion.findOne({ email: userEmail });
        if (!uni)
          return res.status(404).send({ message: "User university not found" });
